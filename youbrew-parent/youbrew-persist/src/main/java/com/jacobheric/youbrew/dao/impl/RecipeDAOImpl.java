...
public class RecipeDAOImpl extends BaseDAOImpl<Recipe, Long> implements
		IRecipeDAO {

	/**
	 * @param recipeCriteria
	...
	public List<Recipe> search(RecipeCriteria recipeCriteria) {
		...
		if (recipeCriteria.getQuery() != null) {
			...
			d.add(Restrictions.like("name", recipeCriteria.getQuery().trim(),
					MatchMode.ANYWHERE));
			d.add(Restrictions.like("tasteNotes", recipeCriteria.getQuery()
					.trim(), MatchMode.ANYWHERE));
			d.add(Restrictions.like("brewNotes", recipeCriteria.getQuery()
					.trim(), MatchMode.ANYWHERE));
